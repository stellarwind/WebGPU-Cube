...
export class Camera {
    ...
    public orbit(yaw: number, pitch: number, distance: number) {
        ...
        // const cameraMatrix = mat4.rotationY(this.yawAngle);
        this.cameraPos = vec3.fromValues(
            Math.sin(this.yawAngle) * distance,
            0,
            Math.cos(this.yawAngle) * distance
        );
        // this.cameraPos = vec3.rotateY(this.cameraPos, [0, 0, 0], yaw);
    }
